...
- 取得は **GitHub API 経由の読み取りのみ**。元 repo を clone して書き込むことは禁止。
  **※ この規約は「索引の取得経路」に対するもの**。**CI の書き戻し経路**(CLAUDE.md 黄金ルール1 の例外)は
  2経路: (1) Claude Action が PR を作るための checkout → 許可パスへの**追加** → PR(organize-loop)
  (2) 決定的スクリプトが PR を作るための checkout → **WBS 限定編集**(ステータストークン置換のみ)→ PR
  (wbs-loop)。手元(開発セッション・executor)からの clone・書き込みは引き続き完全禁止。
