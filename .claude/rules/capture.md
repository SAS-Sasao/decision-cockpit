...
- **status 語彙 = CHECK (status IN ('open','in_progress','done'))**(0006・capture-triage)— **ユーザーの手動トリアージ状態**。UI は**本人行の status のみ UPDATE 可**(processed_at / curated_ref は M5 の整理ループ専用のまま)。未処理バッジ = `user_id スコープ AND processed_at IS NULL AND status = 'open'`。**status と自動整理の関係(消費対象を status='open' に絞るか等)は M5 設計で確定**(docs/design/basic/capture-triage.md §5 の申し送り参照)。
