...
### やらない
- **会話の永続・画面間の共有機構**: overlay を閉じたら会話は破棄(unmount — inline パネルと同じ非永続思想)。/capture の inline パネルとも状態独立。**閉操作を挟まない画面遷移では client メモリ内に会話が生存し得る**(layout 配下の client component — 問い#2 の受容。サーバ永続・外部送信の増加はゼロ)。
...
- SparPanel は現状 client component で props なし・自己完結(fetch/保存/告知文言込み)— overlay 内でそのまま動く(実地確認: "use client"・状態は useState・useRouter 使用は layout 配下でも可)。import は `./capture/spar-panel` 相対形(条件1 でパスをピン — ローカル再実装による二重化を機械排除)。

## 4. リスク・トレードオフ

1. **二重パネル(/capture)**: inline + overlay が同時に開ける — 状態独立で実害なし(受容)。**spar_conclusion を両方から保存すると2レコードになるが、capture_inbox は INSERT-only・1操作=1行の契約どおりで正当**(重複ではない — 受容)。統合は実利用の不満が出てから。
...
1. **overlay・layout**(集計型):
   ...
   grep -Fq './capture/spar-panel' "app/(shell)/spar-overlay.tsx" || fail=1
   ...
   exit "$fail"
   ```
2. **M4 資産・全コードの不変(rev.2 強化 — 広域凍結 diff + 閉包判定)**:
   **2a(広域凍結 diff — M4 条件7 型から layout.tsx のみを除外した形)**:
   ```bash
   fail=0
   git diff --exit-code main -- db lib components scripts fixtures app/api app/login app/auth app/logout app/layout.tsx app/globals.css "app/(shell)/capture" "app/(shell)/page.tsx" "app/(shell)/template.tsx" "app/(shell)/knowledge" "app/(shell)/retro" "app/(shell)/today" "app/(shell)/admin" proxy.ts tests next.config.mjs tsconfig.json package.json package-lock.json .env.example vitest.config.ts || fail=1
   exit "$fail"
   ```
   (lib 全体(auth/search/ingestion/data/spar/ui/db.ts)・db/migrations・components・scripts・globals.css・.env.example を機械被覆 — spar-panel.tsx 含む capture 配下・API・proxy・既存テストは1文字も変えない。)
   **2b(閉包判定 — 追加ファイル含め変更を2ファイルに限定。untracked も検知)**:
   ```bash
   fail=0
...
**手動確認**(機械判定外): 任意の画面でトップバー「壁打ち」→ 右 420px パネルが開き実応答 → 背景クリック / ✕ で閉じる → **閉じてから**再度開くと会話が初期化されている(非永続 — 閉操作を挟まない遷移では生存し得る: 問い#2 受容)→ /capture の inline パネルと独立。
- テスト件数の括弧書き(37 files / 348 tests)は参考値 — 主判定は exit 0。`.env` の不変はハッシュ検証まではしない(禁止事項の宣言 + env -u 形 — 既存受容水準)。
