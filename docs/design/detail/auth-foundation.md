...
| `proxy.ts`(Next 16。旧 middleware.ts) | default / `config` | `auth.middleware({ loginUrl: '/login' })`。matcher = `"/((?!api/auth(?:/|$)|login(?:/|$)|_next/static|_next/image|favicon\\.ico).*)"` — **除外はパス境界付き**(`/api/authx` や `/loginx` は保護対象のまま)。`/login` を明示除外し SDK 内部実装に依存しない。**注記(M1 拡張)**: M1 で `api/sync(?:/|$)` の除外を追加 — 第1層は Route 内の Bearer/admin 検証(docs/design/detail/ingestion-foundation.md §2.4 参照)。**※ 追随注記(capture-spar M4-FIX・2026-07-19)**: SDK 0.4.2-beta の middleware は session 照会(get-session)へ**元リクエストの method/body をそのまま転送**し、cookie キャッシュ路も GET 限定のため、**保護パスへの POST が認証済みでも常に redirect_login(307)になる欠陥**が実機で判明(壁打ち /api/spar・Server Action の POST が全滅)。proxy.ts に **GET 正規化ラッパー**を導入(POST 等の際、url + headers(cookie)を保持した GET 複製を SDK middleware に渡す — セッション判定は cookie のみに依存するため意味論同値・matcher / loginUrl 不変・tests/proxy.test.ts のピンは無変更で緑)。SDK 側で修正されたら不要化を再評価。正典 = docs/design/detail/capture-spar.md §0(M4-FIX 追補) |
