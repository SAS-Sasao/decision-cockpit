...
| 2 | gemini 入力上限の公式確認 | **2,048 tokens で確定**(2026-07-17・ai.google.dev/gemini-api/docs/embeddings — research 比較表に反映済み)。切詰め = **連結後 1,000 文字**(`EMBED_INPUT_MAX_CHARS = 1000` — 日本語最悪 2 tokens/字でも 2,000 < 2,048 で安全・OpenAI 8,191 には余裕) |
| 3 | §5-5 ⊆ 判定の実行形 | §4-5 に確定(grep -RIl の出力を `lib/search/embedding.ts` 単一 or 空と比較) |
| 4 | SQL 全文ピン(embedding_model = $n 束縛形・クランプ) | §2.3/§2.4 で SQL 文字列を固定表記化し §4-3/§4-4 で grep -F ピン。クランプは `Math.max(0,` の grep + テストの範囲 assert |
...
export type KnowledgeHit = {
  ...
  org: string | null; title: string | null; excerpt: string; tags: string[]; similarity: number;
};
export type KnowledgeSearchParams = { q: string; tag?: string; source?: string; from?: string; to?: string; limit?: number };
...
- **検索 SQL(固定表記 — §4-4 で grep -F ピン)**:
  ...
  - 追加フィルタは **$n 追記のみ**(type='decision' 固定 + tag = ANY(tags) / source / occurred_at 範囲 — 全て params 配列)。文字列連結によるユーザー値の埋め込み禁止。
  - `ORDER BY embedding <=> $1::vector ASC` + `LIMIT $n`(既定 8・上限 20 に clamp)。
- **similarity = `Math.max(0, raw_similarity)`**(TS 側クランプ — §4-4 で grep + テストの範囲 assert。降順 assert は非厳密)。
...
- sync-local.ts と同型(dotenv 読込・runEmbedIndex を **remaining=0 まで反復**・各回の EmbedSummary を出力)。DATABASE_URL 差し替えで Neon 本番バックフィルに使用(本番実行は人間承認)。
- **usage コメント等にモデル名リテラルを書かない**(§4-5 の ⊆ 判定対象)。
...
- 構成(MoC isKnowledge 準拠): 検索パネル(input name="q"・GET form・タグチップ 8 個 = topTags・active はリンクでトグル)→ 2列グリッド:
  - 左: 「類似する過去の判断 · N件」+ 結果行カード(日付・org・**類似 pill = `類似 {similarity.toFixed(2)}`**・タイトル・excerpt・タグ pill)。q 空は recent を「最近の判断」として表示。searchError 時はエラーメッセージ + recent。行クリック = `?sel=<id>`(q/tag 保持)。
...
```
(docs 内の正規表現形式言及は接頭辞直後が `[` のためマッチしない — R3 sec で検証済み。変更はこの行のみ・同一コミット則は人間レビュー判定。)
...
| `tests/embed-index.test.ts` | in-memory 行モック(述語ミラー — §0-1)で: 初回 → 全 ok 行 embed・**2回目 → 対象 0 件** / 識別子変更(fixture:A → fixture:B)→ 全行再対象化 / **embedded_at = 読取時 synced_at**(UPDATE 引数の等値 assert)/ EMBED_MAX_ROWS 上限・remaining 計上 / client.embed throw → failed 計上・UPDATE なし / buildEmbedInput(連結順・1,000 文字切詰め・null 要素スキップ) |
| `tests/knowledge-data.test.ts` | searchKnowledge: 類似度降順(**非厳密**)・**全 similarity ∈ [0, 1]**(モックに raw −0.3 / 1.2 相当の距離を含め clamp を実効検証)・各 hit に source/filePath/occurredAt/similarity・$1 の vector 文字列形(`/^\[[-0-9.,eE]+\]$/`)・$2 = 有効識別子・tag/source/date フィルタで params が増える(束縛確認)/ outcomeWindows(境界: anchor ちょうど・+42日直前/直後)/ decisionOutcome(窓割り・0 行窓 null・delta の null 伝播・org 絞り)/ topTags(頻度降順・同数タグ名昇順)/ getKnowledgeData(q 空 → recent・embed throw → searchError=true + recent 返却) |
| `tests/api-sync-embed.test.ts` | run-sync / embed-index / auth をモック: GET(正 secret)→ 200 + `embed` キー / **runEmbedIndex throw → 200 + `embed.error`**(同期本体の成功を妨げない)/ POST(admin)同様 |
...
2. **テスト**: `test -f` ×4(tests/embedding.test.ts / tests/embed-index.test.ts / tests/knowledge-data.test.ts / tests/api-sync-embed.test.ts)+ `npm test` exit 0。
3. **embed-index の SQL ピン**(各 grep -F・exit 0):
   `grep -Fq "(embedding IS NULL OR embedding_model <> $1 OR synced_at > embedded_at)" lib/search/embed-index.ts` /
   `grep -Fq "ORDER BY synced_at ASC" lib/search/embed-index.ts` /
   `grep -Fq "SET embedding = $1::vector, embedding_model = $2, embedded_at = $3" lib/search/embed-index.ts` /
   `grep -Fq "EMBED_INPUT_MAX_CHARS = 1000" lib/search/embed-index.ts`。
4. **検索の SQL ピン**(各 grep -F・exit 0):
   `grep -Fq "WHERE status = 'ok' AND embedding IS NOT NULL AND embedding_model = $2" lib/data/knowledge.ts` /
   `grep -Fq "ORDER BY embedding <=> $1::vector ASC" lib/data/knowledge.ts` /
   `grep -Fq "1 - (embedding <=> $1::vector)" lib/data/knowledge.ts` /
   `grep -Fq "Math.max(0," lib/data/knowledge.ts`。
5. **モデル固定(⊆ 判定・空集合可)**:
   ```bash
   fail=0
...
**手動確認チェックリスト**(機械判定外 — 基本設計 §5 末尾を継承): MoC 目視比較 / 実キーでの初回バックフィル(scripts/embed-local.ts)と日本語クエリの体感品質 / OpenAI キーは **sk-proj- 形式・embeddings 限定 restricted key** で発行 / 切替時はモデル名・キーの**同時交換** + Google 有償枠/データ利用設定の確認 + remaining=0 確認 / 0003 本番適用(ask)と本番バックフィルの時期判断。
...
- **`capture_inbox` のリテラルを lib/search / lib/data/knowledge.ts / app/(shell)/knowledge 配下に書かない(コメント含む — 条件7)**。
- **モデル名リテラル(`text-embedding` / `gemini-embedding`)を lib/search/embedding.ts 以外の app/components/lib/scripts 配下に書かない(embed-local の usage コメント含む — 条件5)**。
