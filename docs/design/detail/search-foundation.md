...
> **※ 追随注記(org-docs-ingestion・2026-07-18)**: SC-04 に **type チップ(判断(既定)/ ナレッジ / すべて)** が追加された。**同 rev.5(OD-FIX)で q 空時の recent 一覧も type・tag を反映する契約に改訂**(本書 §2.4 の「q 空なら最近の判断を表示」は「最近の <type> 一覧(tag 絞り込み付き)」に上書き — 正典 = org-docs-ingestion 詳細 §2.7 rev.5)(knowledge 型の横断検索 — searchKnowledge の IF は無変更・UI の公開のみ)。正典 = docs/design/detail/org-docs-ingestion.md §2.7。
