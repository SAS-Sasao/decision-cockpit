...
- **※ 追随注記(today-view・2026-07-18)**: M3 で allowlist(cc-sier-organization)に WBS パターン `.companies/<org>/docs/secretary/<name>-wbs.md` → AllowMatch kind **"board"** が追加され、board 経路(parseBoard → upsertBoardItems → **board_items テーブル**(0005)— timeline_records とは別テーブル・冪等キー (source, file_path, item_key)・世代識別 commit 列)が新設された。board 経路は **ok/error/skipped に計上しない**(masters 同型)。`RepoSyncSummary` に必須フィールド **`board: { files, items, skippedRows }`** が追加され、/api/sync 応答・sync_state.last_summary へは既存経路で加法拡張として流れる。正典 = docs/design/detail/today-view.md §2.1〜2.3。
