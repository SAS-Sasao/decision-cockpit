...
> 0. **🐛 最優先: /today カンバンでカードを掴んで動かせない・ステータスを変えられない**(2026-07-26 ユーザー報告・
>    作業終了時のため未診断)。次セッションの切り分け手順:
>    (1) **仕様確認から**: 動かせるのは capture 由来カード(「次の一手」「課題」バッジ + アクセント枠)のみ。
>        WBS カードは設計どおり読み取り専用(TBI-1 §1-2)。**capture_inbox は 07-20 の事故で0行**のため、
>        /capture から next_move / issue を1件入れないと掴めるカードが存在しない — まずこれを確認
>    (2) capture カードが表示されている状態で再現するか: **ボタン(→着手 等)は効くか**(効く = D&D のみの
>        問題 / 効かない = Server Action 経路の問題で切り分けが変わる)
>    (3) 環境: ローカル(localhost:3000)か本番(Vercel)か。本番なら **24d2487 のデプロイが Ready 後**か
>        (マージ直後は旧デプロイ = カンバン操作なし)
>    (4) 真のバグなら board.tsx の D&D(draggable / onDragOver preventDefault / drop)とボタンの
>        updateCaptureStatus 呼び出しを実機で追う(front-check は操作系イベント非対象のため手動 + 必要なら
>        一時的な操作 e2e で再現 → 修正 → 削除)
>    (5) 修正は fix/ ブランチ(**必ず main から切る**)→ テスト・e2e 6画面 → merge。
