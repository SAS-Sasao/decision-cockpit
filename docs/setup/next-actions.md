...
> 0. **🐛 /today のサマリーチップ(今日セクション)がカード移動に追随しない**(2026-07-26 ユーザー報告):
>    「オープン」「着手中」の数字は getTodayData の **board_items 生 state の件数**のままで、
>    (a) オーバーレイ(WL-1 の WBS 移動)も (b) capture カードも反映されない — レーンは動くのに数字が不変。
>    **修正方針**: チップの件数を「合成後 columns(applyBoardOverrides 適用済み)+ capture レーン」から
>    数える純関数に置き換える(lib/data/today.ts に summaryFromColumns 等・ユニットテスト付き。
>    表示専用の変更 — 軽量1枚設計 → 3レンズ → 小 goal で1周。TCS-1 と同じ流れ)。
> 0.5 **🤖 codex の運用を本格的に検討したい**(2026-07-26 ユーザー表明 — 旧「案2」を拡張):
>    次セッションの最初のステップは**論点整理の壁打ち**(実装しない):
>    (a) 何に使いたいか(開発の自動化 / 実行時のフロント動的変更 / その他)
>    (b) Claude Code(本プロジェクトの設計→レビュー→judge 統治)との住み分け
>    (c) 安全枠 — 実行時にコードを書き換える形は不採用済み(下記 案2 の3代替: レイアウト設定 DB 化 /
>        開発ループでの AI 改善 / **パラメータのみ操作する SPAR 拡張(最有力)**)。
>        codex を**開発時ツール**として導入する場合も、黄金ルール(SSoT 非接触・guard hooks・凍結テスト)への
>        従属が前提 — 導入形態(CLI / CI / SDK)ごとの攻撃面を比較してから決める。
>    → 方向が決まったら /basic-design で設計に落とす。
